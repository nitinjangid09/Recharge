...
import { getXpressPayoutReport } from '../../api/AuthApi';
    ...
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.7}>
      <View style={styles.cardRow}>
        ...
        <View style={styles.cardCenter}>
          ...
          <Text style={styles.refId}>REF: {txn.referenceId || txn.payoutId}</Text>
...
export default function XpressPayoutReport({ navigation }) {
  ...
  const fetchData = useCallback(async (pageNum = 1, isRefresh = false) => {
    try {
      ...
      const response = await getXpressPayoutReport({ 
        from: fromStr, 
        to: toStr, 
        headerToken: token, 
        page: pageNum, 
      ...
      if (response.success) {
        ...
        setSummary({
          total: response.pagination?.total || (isRefresh ? list.length : data.length + list.length),
          amount: list.reduce((acc, curr) => acc + (curr.amount || 0), 0),
          success: list.filter(t => t.status === 'SUCCESS').length,
        });
      }
    } catch (error) {
      console.log('Error fetching xpress payout report:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const renderItem = ({ item }) => (
    <TxnCard 
      txn={item} 
      onPress={() => navigation.navigate('XpressPayoutReceipt', { txn: item })} 
    />
  );

  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <HeaderBar title="Xpress Payout Report" onBack={() => navigation.goBack()} />
