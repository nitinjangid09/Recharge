...
const STATUS_COLORS = {
  ...
  CLOSED: { bg: Colors.kyc_lockedBg, text: Colors.kyc_textSub },
};

const TicketCard = ({ ticket }) => {
  const [open, setOpen] = useState(false);
  const colors =
    STATUS_COLORS[ticket.status?.toUpperCase()] || { bg: Colors.kyc_lockedBg, text: Colors.kyc_textSub };
  ...
  const loadData = async (pageNum = 1) => {
    ...
    try {
      ...
      const [ticketsResult, profileResult, faqResult, statsResult] = await Promise.allSettled([
        headerToken ? getMySupportRequests({ headerToken, page: pageNum, limit: 5 }) : Promise.resolve(null),
        headerToken ? fetchUserProfile({ headerToken }) : Promise.resolve(null),
        headerToken ? getFAQs({ headerToken }) : Promise.resolve(null),
        headerToken ? getTicketStats({ headerToken }) : Promise.resolve(null),
      ]);
      ...
      if (profileResult.status === "fulfilled" && profileResult.value?.success && profileResult.value?.data?.assignedServices) {
        setAssignedServices(Array.isArray(profileResult.value.data.assignedServices) ? profileResult.value.data.assignedServices : []);
      }
    } catch (err) {
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <TextInput
            ...
            placeholderTextColor={Colors.text_placeholder || "#9CA3AF"}
        ...
        <View style={statsGridStyles.container}>
          ...
          <View style={statsGridStyles.row}>
            <View style={[statsGridStyles.card, { borderLeftColor: Colors.kyc_textSub }]}>
...
const styles = StyleSheet.create({
  ...
  headingBlack: { fontSize: 34, fontFamily: Fonts.Bold, color: Colors.kyc_text, lineHeight: 40 },
  headingRed: { fontSize: 34, fontFamily: Fonts.Bold, color: Colors.red, lineHeight: 40 },
  subheading: { fontSize: 13, fontFamily: Fonts.Medium, color: Colors.kyc_textSub, marginTop: 6, marginBottom: 4 },
  ...
  cardTitle: { fontSize: 16, fontFamily: Fonts.Bold, color: Colors.kyc_text, marginBottom: 4 },
  fieldLabel: {
    fontSize: 11,
  ...
  inputText: { flex: 1, color: Colors.kyc_text, fontSize: 14, fontFamily: Fonts.Medium },
  ...
  quickBtnTxt: { color: Colors.white, fontFamily: Fonts.Bold, fontSize: 10 },
  ...
  ticketId: { fontFamily: Fonts.Bold, fontSize: 14, color: Colors.kyc_text },
  statusBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  statusText: { fontSize: 11, fontFamily: Fonts.Bold, letterSpacing: 0.5 },
  rowLabel: { fontFamily: Fonts.Bold, color: Colors.kyc_text },
  rowText: { fontSize: 13, color: Colors.kyc_textSub, marginTop: 3, fontFamily: Fonts.Medium },
  expandedWrap: { marginTop: 8 },
  divider: { height: 1, backgroundColor: Colors.kyc_lockedBg, marginVertical: 10 },
  caret: { fontSize: 11, color: Colors.red, fontFamily: Fonts.SemiBold, marginTop: 10, textAlign: "right" },
});

const pickerStyles = StyleSheet.create({
  overlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.5)", justifyContent: "flex-end" },
  sheet: { backgroundColor: Colors.white, paddingHorizontal: 20, paddingBottom: Platform.OS === "ios" ? 36 : 20, paddingTop: 12, borderTopLeftRadius: 26, borderTopRightRadius: 26, maxHeight: "65%" },
  handle: { width: 40, height: 4, backgroundColor: Colors.kyc_lockedBorder, borderRadius: 2, alignSelf: "center", marginBottom: 16 },
  title: { fontSize: 17, fontFamily: Fonts.Bold, color: Colors.kyc_text, marginBottom: 14 },
  emptyText: { color: "#9CA3AF", fontSize: 14, textAlign: "center", marginVertical: 30, fontFamily: Fonts.Medium },
  row: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: Colors.kyc_lockedBg },
  rowText: { fontSize: 15, color: Colors.kyc_text, fontFamily: Fonts.Medium },
  ...
  closeBtn: { marginTop: 16, paddingVertical: 14, backgroundColor: Colors.kyc_lockedBg, borderRadius: 14, alignItems: "center" },
  closeTxt: { color: Colors.kyc_textSub, fontFamily: Fonts.Bold, fontSize: 14 },
});

const faqStyles = StyleSheet.create({
  container: { paddingHorizontal: 20, marginTop: 10, marginBottom: 10 },
  title: { fontSize: 18, fontFamily: Fonts.Bold, color: Colors.kyc_text, marginBottom: 14 },
  item: {
    backgroundColor: Colors.white,
    borderRadius: 16,
    padding: 16,
    marginBottom: 10,
    borderWidth: 1,
    borderColor: Colors.kyc_lockedBg,
    elevation: 1,
    shadowColor: Colors.black,
    shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.05,
  ...
  question: { fontSize: 14, fontFamily: Fonts.SemiBold, color: Colors.kyc_text, flex: 1, paddingRight: 10 },
  answer: { marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: Colors.kyc_lockedBg },
  answerTxt: { fontSize: 13, color: Colors.kyc_textSub, fontFamily: Fonts.Medium, lineHeight: 20 },
});

const historyStyles = StyleSheet.create({
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginHorizontal: 20, marginTop: 10, marginBottom: 12 },
  title: { fontSize: 18, fontFamily: Fonts.Bold, color: Colors.kyc_text },
});

const paginationStyles = StyleSheet.create({
  card: { backgroundColor: Colors.white, marginHorizontal: 20, marginTop: 5, marginBottom: 20, padding: 12, borderRadius: 16, flexDirection: "row", alignItems: "center", justifyContent: "space-between", elevation: 2, shadowColor: Colors.black, shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 4 },
  btn: { backgroundColor: Colors.red, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10, minWidth: 80, alignItems: "center" },
  btnDisabled: { backgroundColor: Colors.kyc_lockedBorder, opacity: 0.8 },
  btnText: { color: Colors.white, fontSize: 12, fontFamily: Fonts.Bold, letterSpacing: 0.5 },
  infoContainer: { paddingHorizontal: 12 },
  infoText: { fontSize: 14, fontFamily: Fonts.Bold, color: Colors.kyc_text },
});

const statsGridStyles = StyleSheet.create({
  container: { paddingHorizontal: 20, marginVertical: 10, gap: 8 },
  row: { flexDirection: "row", gap: 8 },
  card: { flex: 1, backgroundColor: Colors.white, borderRadius: 12, padding: 10, borderLeftWidth: 3, elevation: 2, shadowColor: Colors.black, shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2 },
  val: { fontSize: 16, fontFamily: Fonts.Bold, color: Colors.kyc_text },
  lbl: { fontSize: 8, fontFamily: Fonts.Bold, color: Colors.kyc_textSub, letterSpacing: 0.8, marginTop: 1 },
});
