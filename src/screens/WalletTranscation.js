...
const getTxnDate = (item) => item.createdAt || item.date || item.created_at || item.txnDate || item.transactionDate || item.addDate;
const safeDateStr = (item) => formatApiDate(getTxnDate(item));
...
const downloadReceipt = async (item) => {
  try {
    ...
    const dateStr = safeDateStr(item);
    const amtStr = safeAmount(item.amount);
    let RNHTMLtoPDF;
    ...
    if (RNHTMLtoPDF) {
      const html = `<html><head><meta charset="utf-8"/><style>
        ...
        <div style="text-align:center;margin:16px 0;"><div class="badge">${typeLabel}</div>
          <div class="amount">${amtPrefix}${amtStr}</div></div>
        <table>
          ...
          <tr><td>Opening Balance</td><td>₹${safeAmount(item.openingBalance)}</td></tr>
          <tr><td>Closing Balance</td><td>₹${safeAmount(item.closingBalance)}</td></tr>
        </table>
    ...
    await Share.share({
      message: `── WALLET RECEIPT ──\nType: ${typeLabel}\nAmount: ${amtPrefix}${amtStr}\nRef: ${item.referenceId ?? '—'}\nDate: ${dateStr}\nDesc: ${item.description ?? '—'}\nWallet: ${item.wallet?.toUpperCase() ?? 'MAIN'}\nOpening: ₹${safeAmount(item.openingBalance)}\nClosing: ₹${safeAmount(item.closingBalance)}`,
...
const FST = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: sc(20), paddingVertical: vs(14), borderBottomWidth: 1, borderBottomColor: T.textHint },
  ...
  // Nav sidebar
  navCol: { width: sc(82), borderRightWidth: 1, borderRightColor: T.textHint, paddingTop: vs(10) },
  ...
  optRowActive: { backgroundColor: T.brandLight, marginHorizontal: -sc(4), paddingHorizontal: sc(8) },
  optIconBox: { width: sc(32), height: sc(32), borderRadius: sc(9), backgroundColor: T.divider, alignItems: 'center', justifyContent: 'center' },
  ...
  // Footer
  footer: { paddingHorizontal: sc(20), paddingTop: vs(12), paddingBottom: Platform.OS === 'ios' ? vs(34) : vs(16), borderTopWidth: 1, borderTopColor: T.textHint },
...
const TxnDetailSheet = ({ visible, item, onClose }) => {
  ...
  const isFailed = item.status && ['failed', 'failure'].includes(String(item.status).toLowerCase());
  const isPending = item.status && ['pending', 'processing'].includes(String(item.status).toLowerCase());
  const statusIcon = isFailed ? 'close-circle' : (isPending ? 'clock-outline' : 'check-circle');
  const statusColor = isFailed ? T.danger : (isPending ? T.warning : T.success);
  const statusLabel = isFailed ? 'Transaction Failed' : (isPending ? 'Transaction Pending' : 'Transaction Successful');

  const amtStr = safeAmount(item.amount);

  const rows = [
    { label: 'DESCRIPTION', val: item.description ?? '—', icon: 'text-box-outline' },
    { label: 'STATUS', val: item.status ? String(item.status).toUpperCase() : (isDebit ? 'DEBITED' : 'CREDITED'), valColor: statusColor, icon: 'circle-slice-8' },
    { label: 'REFERENCE ID', val: item.referenceId ?? '—', icon: 'tag-outline' },
    { label: 'DATE & TIME', val: safeDateStr(item), icon: 'calendar-blank' },
    { label: 'AMOUNT', val: `₹${amtStr}`, valColor: amtColor, icon: 'credit-card-outline' },
    { label: 'WALLET', val: item.wallet?.toUpperCase() ?? 'MAIN', icon: 'wallet-outline' },
    { label: 'OPENING BALANCE', val: `₹${safeAmount(item.openingBalance)}`, icon: 'bank-outline' },
    { label: 'CLOSING BALANCE', val: `₹${safeAmount(item.closingBalance)}`, valColor: amtColor, icon: 'bank-check-outline' },
    ...
    <Modal transparent visible={visible} animationType="none" onRequestClose={onClose}>
      ...
      <Animated.View style={[DET.sheet, { transform: [{ translateY: slideA }] }]}>
        ...
        <View style={{ alignItems: 'center', marginTop: vs(24), marginBottom: vs(16) }}>
          ...
          <Text style={{ fontSize: rs(16), fontFamily: Fonts.Bold, color: T.textPri, marginTop: vs(12) }}>
            {statusLabel}
          </Text>
          <Text style={{ fontSize: rs(26), fontFamily: Fonts.Bold, color: amtColor, marginTop: vs(6), letterSpacing: -0.5 }}>
            {amtPrefix}{amtStr}
          </Text>
        </View>
    ...
    <View style={TXC.card}>
      ...
      <View style={TXC.top}>
        ...
        <View style={TXC.right}>
          <Text style={[TXC.amount, { color: accent }]}>{prefix}{safeAmount(item.amount)}</Text>
      ...
      <View style={TXC.balRow}>
        <View style={TXC.balItem}>
          ...
          <Text style={TXC.balVal}>₹{safeAmount(item.openingBalance)}</Text>
        </View>
        ...
        <View style={[TXC.balItem, { alignItems: 'flex-end' }]}>
          ...
          <Text style={[TXC.balVal, { color: accent }]}>₹{safeAmount(item.closingBalance)}</Text>
        </View>
      </View>
      ...
      <View style={TXC.footRow}>
        ...
        <Text style={TXC.footDate}>{safeDateStr(item)}</Text>
        {item.wallet && (
...
const TXC = StyleSheet.create({
  ...
  balArrow: { width: sc(24), height: sc(24), borderRadius: sc(12), alignItems: 'center', justifyContent: 'center' },
